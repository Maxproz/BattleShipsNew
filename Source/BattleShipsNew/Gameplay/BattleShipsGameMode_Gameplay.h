...
#include "BattleShipsGameMode_Gameplay.generated.h"
 ...
 */
UCLASS()
class BATTLESHIPSNEW_API ABattleShipsGameMode_Gameplay : public AGameModeBase
{
	GENERATED_BODY()
	
	ABattleShipsGameMode_Gameplay(const FObjectInitializer& ObjectInitializer);

	// NOTE: I need to add DETAILED notes to this class and the corresponding game state class if I want to have any hope
	// of not getting completly confused at how I am tracking data and then run in circles for many hours.

	// Things I should track in here for the battleship game that will make overall organization easier.
	// TODO: IMPORTANT: do this next.
	// When I add something here I should also have the game-state open and think about the functions that connect them and make notes.
	
	// Start with creating some necessary private variables that are needed for the game of battleship
...
public:
	...
	// This is where we will initalize the game timer for the game mode.
	...
	virtual bool CanDealDamage(class APlayerState_Gameplay* DamageInstigator, APlayerState_Gameplay* DamagedPlayer) const;
...
protected:
	...
	/** number of teams */
	// NumTeams is always 1 player 1 AI
	int32 NumTeams;

	/** best team */
	// This should probably just be bool bDidPlayerWin or somehow use this int to determine player or AI
	int32 WinnerTeam;

	/** There is probably unneeded in single player */
	...
	virtual bool IsWinner(class APlayerState_Gameplay* PlayerState) const;

	//void InitSeamlessTravelPlayer(AController * NewController);

	// TODO: Fix this function to check to see if its possible to place a ship at those squares
...
public:
	...
	virtual TSubclassOf<class ABattleShipsGameMode_Gameplay> GetGameModeClass(const FString& MapName, const FString& Options, const FString& Portal) const;
